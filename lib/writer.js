 ...
 * @param {String} options.issueTrackerUrl - URL that will be used when linking issues
...
exports.markdown = function (version, commits, options) {
  ...
  return new Bluebird(function (resolve) {
    var tagDate =
      'git log -1 --pretty=format:"%ad" --date=short `git describe --abbrev=0`';
    return resolve(CP.execAsync(tagDate));
  })
  .catch(function () {
    throw new Error('no tag date found');
  })
  .then(function (date) {
    if (options.major) {
      heading = '##';
      ...
      types[type][category].forEach(function (commit) {
        ...
        if (options.issueTrackerUrl) {
          subject = subject.replace(PR_REGEX, function (pr) {
            return (
              '[' + pr + '](' + options.issueTrackerUrl + pr.slice(1) + ')'
            );
          });
        }
